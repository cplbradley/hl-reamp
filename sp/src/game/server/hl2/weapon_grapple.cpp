...
void CGrappleHook::HookedThink(void)
{
	...
	SetNextThink(gpGlobals->curtime + 0.005f); //0.1f
	...
	if (m_hOwner->m_bHook){
		temp_multiplier = 1;
		m_hPlayer->SetAbsVelocity(tempVec1*temp_multiplier * 250);//400
	}
	else
	{
		m_hPlayer->SetAbsVelocity(tempVec1*temp_multiplier * 1600);//400
	}
}
//-----------------------------------------------------------------------------
// Purpose:
...
void CWeaponGrapple::FireHook(void)
{
	...
	{
		pHook->SetAbsVelocity(vecAiming * BOLT_AIR_VELOCITY * 2);
	}
	m_hHook = pHook;
#endif
