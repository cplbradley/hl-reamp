...
ConVar	sk_bloat_health("sk_bloat_health", "30");

envelopePoint_t envBloatMoanVolumeFast[] =
{
	{ 7.0f, 7.0f,
	0.1f, 0.1f,
	},
...
envelopePoint_t envBloatMoanVolume[] =
{
	{ 1.0f, 1.0f,
	0.1f, 0.1f,
	},
...
envelopePoint_t envBloatMoanVolumeLong[] =
{
	{ 1.0f, 1.0f,
	0.3f, 0.5f,
	},
...
envelopePoint_t envBloatMoanIgnited[] =
{
	{ 1.0f, 1.0f,
	0.5f, 1.0f,
	},
...
class CNPCBloat : public CAI_BlendingHost<CNPC_BaseZombie>
{
	DECLARE_DATADESC();
	DECLARE_CLASS(CNPCBloat, CAI_BlendingHost<CNPC_BaseZombie>);
...
LINK_ENTITY_TO_CLASS(npc_bloat, CNPCBloat);

//---------------------------------------------------------
//---------------------------------------------------------
const char *CNPCBloat::pMoanSounds[] =
{
...
int ACT_BLOAT_TANTRUM;
int ACT_BLOAT_WALLPOUND;

BEGIN_DATADESC(CNPCBloat)
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPCBloat::Precache(void)
{
	BaseClass::Precache();

	PrecacheModel("models/zombie/bloat.mdl");
...
void CNPCBloat::Spawn(void)
{
	...
	m_iHealth = sk_bloat_health.GetFloat();
	m_flFieldOfView = 0.2;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPCBloat::PrescheduleThink(void)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPCBloat::SelectSchedule(void)
{
...
//-----------------------------------------------------------------------------
void CNPCBloat::FootstepSound(bool fRightFoot)
{
	if (fRightFoot)
	{
...
//-----------------------------------------------------------------------------
void CNPCBloat::FootscuffSound(bool fRightFoot)
{
	if (fRightFoot)
	{
...
//-----------------------------------------------------------------------------
void CNPCBloat::AttackHitSound(void)
{
...
//-----------------------------------------------------------------------------
void CNPCBloat::AttackMissSound(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPCBloat::PainSound(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPCBloat::DeathSound(const CTakeDamageInfo &info)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPCBloat::AlertSound(void)
{
...
//-----------------------------------------------------------------------------
const char *CNPCBloat::GetMoanSound(int nSound)
{
...
//-----------------------------------------------------------------------------
void CNPCBloat::AttackSound(void)
{
...
//-----------------------------------------------------------------------------
const char *CNPCBloat::GetHeadcrabClassname(void)
{
	return "npc_headcrab";
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char *CNPCBloat::GetHeadcrabModel(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *CNPCBloat::GetLegsModel(void)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char *CNPCBloat::GetTorsoModel(void)
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPCBloat::SetZombieModel(void)
{
	Hull_t lastHull = GetHullType();
	SetModel("models/zombie/bloat.mdl");
	SetHullType(HULL_HUMAN);
	SetModelScale(1.25f, 0.0f);
...
void CNPCBloat::Event_Killed(const CTakeDamageInfo &info)
{
	for (int i = 0; i < 2; i++)
	{
		...
		Vector vecSrc = (vecRndSrc + Vector(0, 0, 48));
		pSoul->SetAbsOrigin(vecSrc);
		pSoul->Spawn();
	}
	//RadiusDamage(CTakeDamageInfo(this, this, 40, DMG_PARALYZE | DMG_POISON), GetAbsOrigin(), 128, CLASS_ANTLION, NULL);
	Vector Itemsrc = (GetAbsOrigin() + Vector(0, 0, 48));
	SetModel("models/zombie/classic_legs.mdl");
	DropItem("item_ammo_smg1", Itemsrc + RandomVector(-4, 4), RandomAngle(0, 360));
	DropItem("item_drop_buckshot", Itemsrc + RandomVector(-4, 4), RandomAngle(0, 360));
	DropItem("item_ammo_ar2", Itemsrc + RandomVector(-4, 4), RandomAngle(0, 360));
	DropItem("item_rpg_drop", Itemsrc + RandomVector(-4, 4), RandomAngle(0, 360));
	DropItem("item_ammo_crossbow", Itemsrc + RandomVector(-4, 4), RandomAngle(0, 360));
	DropItem("item_batterydrop", Itemsrc + RandomVector(-4, 4), RandomAngle(0, 360));
	DropItem("item_batterydrop", Itemsrc + RandomVector(-4, 4), RandomAngle(0, 360));
	DropItem("item_batterydrop", Itemsrc + RandomVector(-4, 4), RandomAngle(0, 360));
	DropItem("item_batterydrop", Itemsrc + RandomVector(-4, 4), RandomAngle(0, 360));
	DropItem("item_batterydrop", Itemsrc + RandomVector(-4, 4), RandomAngle(0, 360));
	DropItem("item_healthvial", Itemsrc + RandomVector(-4, 4), RandomAngle(0, 360));
	DropItem("item_healthvial", Itemsrc + RandomVector(-4, 4), RandomAngle(0, 360));
	DropItem("item_healthvial", Itemsrc + RandomVector(-4, 4), RandomAngle(0, 360));


	DispatchParticleEffect("agib_sploosh", Itemsrc, GetAbsAngles(), this);
	BaseClass::Event_Killed(info);
}
//---------------------------------------------------------
// Classic zombie only uses moan sound if on fire.
//---------------------------------------------------------
void CNPCBloat::MoanSound(envelopePoint_t *pEnvelope, int iEnvelopeSize)
{
	if (IsOnFire())
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPCBloat::ShouldBecomeTorso(const CTakeDamageInfo &info, float flDamageThreshold)
{
	if (IsSlumped())
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPCBloat::GatherConditions(void)
{
...
int CNPCBloat::SelectFailSchedule(int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode)
{
...
int CNPCBloat::TranslateSchedule(int scheduleType)
{
...
Activity CNPCBloat::NPC_TranslateActivity(Activity newActivity)
{
	...
	if (m_fIsTorso && (newActivity == ACT_BLOAT_TANTRUM))
		return ACT_IDLE;
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPCBloat::OnStateChange(NPC_STATE OldState, NPC_STATE NewState)
{
...
void CNPCBloat::StartTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_ZOMBIE_EXPRESS_ANGER:
	{
		...
		{
			SetIdealActivity((Activity)ACT_BLOAT_TANTRUM);
		}
		else
		{
			TaskComplete();
...
void CNPCBloat::RunTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_ZOMBIE_ATTACK_DOOR:
...
bool CNPCBloat::OnObstructingDoor(AILocalMoveGoal_t *pMoveGoal, CBaseDoor *pDoor,
...
Activity CNPCBloat::SelectDoorBash()
{
	if (random->RandomInt(1, 3) == 1)
		return ACT_MELEE_ATTACK1;
	return (Activity)ACT_BLOAT_WALLPOUND;
}
...
//---------------------------------------------------------
void CNPCBloat::Ignite(float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner)
{
	if (!IsOnFire() && IsAlive())
	{
		...
		if (!UTIL_IsLowViolence())
		{
			...
			MoanSound(envBloatMoanIgnited, ARRAYSIZE(envBloatMoanIgnited));
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPCBloat::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo)
{
#ifndef HL2_EPISODIC
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPCBloat::IsHeavyDamage(const CTakeDamageInfo &info)
{
#ifdef HL2_EPISODIC
...
bool CNPCBloat::IsSquashed(const CTakeDamageInfo &info)
{
	if (GetHealth() > 0)
	{
		return false;
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPCBloat::BuildScheduleTestBits(void)
{
...
AI_BEGIN_CUSTOM_NPC(npc_bloat, CNPCBloat)
...
DECLARE_ACTIVITY(ACT_BLOAT_TANTRUM);
DECLARE_ACTIVITY(ACT_BLOAT_WALLPOUND);
...
"	Tasks"
"		TASK_SET_ACTIVITY				ACTIVITY:ACT_BLOAT_TANTRUM"
...
"		TASK_PLAY_SEQUENCE				ACTIVITY:ACT_BLOAT_TANTRUM" /* placeholder until frustration/rage/fence shake animation available */
""
"	Interrupts"
"		COND_ZOMBIE_RELEASECRAB"
"		COND_ENEMY_DEAD"
