...
ConVar	sk_legs_health("sk_legs_health", "0");
...
class CNPCLegs : public CAI_BlendingHost<CNPC_BaseZombie>
{
	DECLARE_DATADESC();
	DECLARE_CLASS(CNPCLegs, CAI_BlendingHost<CNPC_BaseZombie>);
...
LINK_ENTITY_TO_CLASS(npc_legs, CNPCLegs);
//LINK_ENTITY_TO_CLASS(npc_zombie_torso, CNPCLegs);

//---------------------------------------------------------
//---------------------------------------------------------
const char *CNPCLegs::pMoanSounds[] =
{
...
int ACT_LEGS_TANTRUM;
int ACT_LEGS_WALLPOUND;

BEGIN_DATADESC(CNPCLegs)
...
void CNPCLegs::Precache(void)
{
	...
	PrecacheModel("models/zombie/legs.mdl");
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPCLegs::Spawn(void)
{
	Precache();

	if (FClassnameIs(this, "npc_legs"))
	{
		m_fIsTorso = false;
	}
	else
	...
	m_iHealth = sk_legs_health.GetFloat();
	m_flFieldOfView = 0.2;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPCLegs::PrescheduleThink(void)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPCLegs::SelectSchedule(void)
{
...
//-----------------------------------------------------------------------------
void CNPCLegs::FootstepSound(bool fRightFoot)
{
	if (fRightFoot)
	{
...
//-----------------------------------------------------------------------------
void CNPCLegs::FootscuffSound(bool fRightFoot)
{
	if (fRightFoot)
	{
...
//-----------------------------------------------------------------------------
void CNPCLegs::AttackHitSound(void)
{
...
//-----------------------------------------------------------------------------
void CNPCLegs::AttackMissSound(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPCLegs::PainSound(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPCLegs::DeathSound(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
const char *CNPCLegs::GetMoanSound(int nSound)
{
	return  NULL;//pMoanSounds[nSound % ARRAYSIZE(pMoanSounds)];
}

//-----------------------------------------------------------------------------
// Purpose: Play a random idle sound.
//-----------------------------------------------------------------------------
void CNPCLegs::IdleSound(void)
{
...
//-----------------------------------------------------------------------------
void CNPCLegs::AttackSound(void)
{
...
//-----------------------------------------------------------------------------
const char *CNPCLegs::GetHeadcrabClassname(void)
{
	return "npc_headcrab";
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char *CNPCLegs::GetHeadcrabModel(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *CNPCLegs::GetLegsModel(void)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char *CNPCLegs::GetTorsoModel(void)
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPCLegs::SetZombieModel(void)
{
	...
	{
		SetModel("models/zombie/legs.mdl");
...
//---------------------------------------------------------
void CNPCLegs::MoanSound(envelopePoint_t *pEnvelope, int iEnvelopeSize)
{
	if (IsOnFire())
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPCLegs::ShouldBecomeTorso(const CTakeDamageInfo &info, float flDamageThreshold)
{
/*	if (IsSlumped())
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPCLegs::GatherConditions(void)
{
...
int CNPCLegs::SelectFailSchedule(int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode)
{
...
int CNPCLegs::TranslateSchedule(int scheduleType)
{
...
Activity CNPCLegs::NPC_TranslateActivity(Activity newActivity)
{
	...
	if (m_fIsTorso && (newActivity == ACT_LEGS_TANTRUM))
		return ACT_IDLE;
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPCLegs::OnStateChange(NPC_STATE OldState, NPC_STATE NewState)
{
...
void CNPCLegs::StartTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_ZOMBIE_EXPRESS_ANGER:
	{
		...
		{
			SetIdealActivity((Activity)ACT_LEGS_TANTRUM);
		}
		else
		{
			TaskComplete();
...
void CNPCLegs::RunTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_ZOMBIE_ATTACK_DOOR:
...
bool CNPCLegs::OnObstructingDoor(AILocalMoveGoal_t *pMoveGoal, CBaseDoor *pDoor,
...
Activity CNPCLegs::SelectDoorBash()
{
	if (random->RandomInt(1, 3) == 1)
		return ACT_MELEE_ATTACK1;
	return (Activity)ACT_LEGS_WALLPOUND;
}
...
//---------------------------------------------------------
void CNPCLegs::Ignite(float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner)
{
	if (!IsOnFire() && IsAlive())
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPCLegs::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo)
{
#ifndef HL2_EPISODIC
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPCLegs::IsHeavyDamage(const CTakeDamageInfo &info)
{
#ifdef HL2_EPISODIC
...
bool CNPCLegs::IsSquashed(const CTakeDamageInfo &info)
{
	if (GetHealth() > 0)
	{
		return false;
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPCLegs::BuildScheduleTestBits(void)
{
...
AI_BEGIN_CUSTOM_NPC(npc_legs, CNPCLegs)
...
DECLARE_ACTIVITY(ACT_LEGS_TANTRUM);
DECLARE_ACTIVITY(ACT_LEGS_WALLPOUND);
