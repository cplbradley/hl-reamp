...
DEFINE_FIELD(m_bDrawHud,FIELD_BOOLEAN), //turns out we don't want to save the bool, we just want it to be false on reload
...
{
	m_iTimeLeft--;
	if (UTIL_GetLocalPlayer() && !UTIL_GetLocalPlayer()->IsAlive())
		m_bDrawHud = false;
...
void CHLRFreqShifter::Touch(CBaseEntity *pOther)
{
	if (pOther->IsPlayer())
	{
		m_OnPickup.FireOutput(this, pOther);
		SetSolid(SOLID_NONE);
		RemoveSolidFlags(FSOLID_TRIGGER);
		//m_hSpitEffect->StopParticleSystem();
