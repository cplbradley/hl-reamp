...
IMPLEMENT_NETWORKCLASS_ALIASED(Missile, DT_Missile)

BEGIN_NETWORK_TABLE(CMissile, DT_Missile)
// Client specific.
#ifdef CLIENT_DLL
...
CMissile *CMissile::Create(const Vector &vecOrigin, const QAngle &vecAngles, edict_t *pentOwner = NULL)
{
	...
	CMissile *pMissile = static_cast<CMissile*>(CBaseEntity::Create("rpg_missile", vecOrigin, vecAngles, CBaseEntity::Instance(pentOwner)));
...
#endif
#ifdef CLIENT_DLL
void CMissile::PostDataUpdate(DataUpdateType_t type)
{
