...
class CLogicMusicPlayer : public CLogicalEntity
{
	DECLARE_CLASS(CLogicMusicPlayer, CLogicalEntity);
private:
...
LINK_ENTITY_TO_CLASS(logic_random_music, CLogicMusicPlayer);


BEGIN_DATADESC(CLogicMusicPlayer)
...
// Purpose: Called before spawning, after key values have been set.
//-----------------------------------------------------------------------------
void CLogicMusicPlayer::Spawn(void)
{
	m_nLastShuffleCase = -1;
	Precache();
}

void CLogicMusicPlayer::Precache(void)
{
	for (int i = 0; i < MAX_LOGIC_CASES; i++)
	{
		if (m_nCase[i] != NULL_STRING)
		{
			char *szSoundFile = (char *)STRING(m_nCase[i]);
...
//				We use a variant so that we can convert any input type to a string.
//-----------------------------------------------------------------------------
void CLogicMusicPlayer::InputValue(inputdata_t &inputdata)
{
	const char *pszValue = inputdata.value.String();
	for (int i = 0; i < MAX_LOGIC_CASES; i++)
	{
...
// NOTE: assumes pnMap points to an array of MAX_LOGIC_CASES
//-----------------------------------------------------------------------------
int CLogicMusicPlayer::BuildCaseMap(unsigned char *puchCaseMap)
{
	memset(puchCaseMap, 0, sizeof(unsigned char) * MAX_LOGIC_CASES);
...
// Purpose: Makes the case statement choose a case at random.
//-----------------------------------------------------------------------------
void CLogicMusicPlayer::InputPickRandom(inputdata_t &inputdata)
{
	unsigned char uchCaseMap[MAX_LOGIC_CASES];
	int nNumCases = BuildCaseMap(uchCaseMap);
	...
	if (nNumCases > 0)
	{
		...
		if (nCase < MAX_LOGIC_CASES)
		{
			...
			char *szSoundFile = (char *)STRING(m_nCase[nCase]);
			src->m_pSoundName = szSoundFile;
			EmitSound(filter, entindex(), *src);
...
void CLogicMusicPlayer::CheckPlaybackStatus(void)
{
...
void CLogicMusicPlayer::PickRandom(void)
{
	unsigned char uchCaseMap[MAX_LOGIC_CASES];
	int nNumCases = BuildCaseMap(uchCaseMap);
...
// Purpose: Makes the case statement choose a case at random.
//-----------------------------------------------------------------------------
void CLogicMusicPlayer::InputPickRandomShuffle(inputdata_t &inputdata)
{
	int nAvoidCase = -1;
	int nCaseCount = m_nShuffleCases;
