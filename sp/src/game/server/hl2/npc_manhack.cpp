...
#define MANHACK_BODYGROUP_BLADE	0
#define MANHACK_BODYGROUP_BLUR	0
#define MANHACK_BODYGROUP_OFF	0
#define MANHACK_BODYGROUP_ON	0
...
void CNPC_Manhack::StartEye(void)
{
	...
	if (m_pLightGlow == NULL)
	{
		...
		m_pLightGlow->SetScale(1.5f, 0.1f);
		m_pLightGlow->SetAsTemporary();
	}
	//DispatchParticleEffect("skull_flame", GetAbsOrigin(), GetAbsAngles(), this);
}
...
void CNPC_Manhack::SetEyeState(int state)
{
	...
	case MANHACK_EYE_STATE_CHARGE:
	{
		...
		if (m_pLightGlow)
		{
			...
			m_pLightGlow->SetScale(1.5f, 0.5f);
			m_pLightGlow->SetBrightness(164, 0.1f);
