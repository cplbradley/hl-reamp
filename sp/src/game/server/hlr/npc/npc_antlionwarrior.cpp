...
#include "npc_antlionwarrior.h"
// memdbgon must be the last include file in a .cpp file!!!
#include "tier0/memdbgon.h"

Activity ACT_ANTLIONWARRIOR_SEARCH;
Activity ACT_ANTLIONWARRIOR_PEEK_FLINCH;
Activity ACT_ANTLIONWARRIOR_PEEK_ENTER;
Activity ACT_ANTLIONWARRIOR_PEEK_EXIT;
Activity ACT_ANTLIONWARRIOR_PEEK1;
Activity ACT_ANTLIONWARRIOR_BARK;
Activity ACT_ANTLIONWARRIOR_PEEK_SIGHTED;
Activity ACT_ANTLIONWARRIOR_SHOVE_PHYSOBJECT;
Activity ACT_ANTLIONWARRIOR_FLINCH_LIGHT;
Activity ACT_ANTLIONWARRIOR_UNBURROW;
Activity ACT_ANTLIONWARRIOR_ROAR;
Activity ACT_ANTLIONWARRIOR_RUN_HURT;

// Flinches
Activity ACT_ANTLIONWARRIOR_PHYSHIT_FR;
Activity ACT_ANTLIONWARRIOR_PHYSHIT_FL;
Activity ACT_ANTLIONWARRIOR_PHYSHIT_RR;
Activity ACT_ANTLIONWARRIOR_PHYSHIT_RL;

// Charge
Activity ACT_ANTLIONWARRIOR_CHARGE_START;
Activity ACT_ANTLIONWARRIOR_CHARGE_CANCEL;
Activity ACT_ANTLIONWARRIOR_CHARGE_RUN;
Activity ACT_ANTLIONWARRIOR_CHARGE_CRASH;
Activity ACT_ANTLIONWARRIOR_CHARGE_STOP;
...
int AE_ANTLIONWARRIOR_SHOVE_PHYSOBJECT;
int AE_ANTLIONWARRIOR_SHOVE;
int AE_ANTLIONWARRIOR_FOOTSTEP_LIGHT;
int AE_ANTLIONWARRIOR_FOOTSTEP_HEAVY;
int AE_ANTLIONWARRIOR_CHARGE_EARLYOUT;
int AE_ANTLIONWARRIOR_VOICE_GROWL;
int AE_ANTLIONWARRIOR_VOICE_BARK;
int AE_ANTLIONWARRIOR_VOICE_PAIN;
int AE_ANTLIONWARRIOR_VOICE_SQUEEZE;
int AE_ANTLIONWARRIOR_VOICE_SCRATCH;
int AE_ANTLIONWARRIOR_VOICE_GRUNT;
...
envelopePoint_t envAntlionWarriorFastGrowl[] =
{
	{ 1.0f, 1.0f,
	0.2f, 0.4f,
	},
...
//Bark 1 (Growl High)
envelopePoint_t envAntlionWarriorBark1[] =
{
	{ 1.0f, 1.0f,
	0.1f, 0.2f,
	},
...
//Bark 2 (Confused)
envelopePoint_t envAntlionWarriorBark2[] =
{
	{ 1.0f, 1.0f,
	0.1f, 0.2f,
	},
...
//Pain
envelopePoint_t envAntlionWarriorPain1[] =
{
	{ 1.0f, 1.0f,
	0.1f, 0.2f,
	},
...
//Squeeze (High Growl)
envelopePoint_t envAntlionWarriorSqueeze[] =
{
	{ 1.0f, 1.0f,
	0.1f, 0.2f,
	},
...
//Scratch (Low Growl)
envelopePoint_t envAntlionWarriorScratch[] =
{
	{ 1.0f, 1.0f,
	0.4f, 0.6f,
	},
...
//Grunt
envelopePoint_t envAntlionWarriorGrunt[] =
{
	{ 0.6f, 1.0f,
	0.1f, 0.2f,
	},
...
envelopePoint_t envAntlionWarriorGrunt2[] =
{
	{ 0.2f, 0.4f,
	0.1f, 0.2f,
	},
...
impactdamagetable_t gAntlionWarriorImpactDamageTable =
{
...
//-----------------------------------------------------------------------------
const impactdamagetable_t &CNPC_AntlionWarrior::GetPhysicsImpactDamageTable(void)
{
	return gAntlionWarriorImpactDamageTable;
}
...
CNPC_AntlionWarrior::CNPC_AntlionWarrior(void)
{
...
LINK_ENTITY_TO_CLASS(npc_antlionwarrior, CNPC_AntlionWarrior);

IMPLEMENT_SERVERCLASS_ST(CNPC_AntlionWarrior, DT_NPC_AntlionWarrior)
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::UpdateOnRemove(void)
{
	DestroyGlows();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::Precache(void)
{
	PrecacheModel(ANTLIONWARRIOR_MODEL);

	PrecacheScriptSound("NPC_AntlionGuard.Shove");
	PrecacheScriptSound("NPC_AntlionGuard.HitHard");

	if (HasSpawnFlags(SF_ANTLIONWARRIOR_INSIDE_FOOTSTEPS))
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::DestroyGlows(void)
{
	if (m_hCaveGlow[0])
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::CreateGlow(CSprite **pSprite, const char *pAttachName)
{
	if (pSprite == NULL)
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::Activate(void)
{
	BaseClass::Activate();
	...
	while ((pObject = gEntList.FindEntityInSphere(pObject, WorldSpaceCenter(), 2500)) != NULL)
	{
		...
		if ((pPhysObj == NULL) || (pPhysObj->GetMass() > ANTLIONWARRIOR_MAX_OBJECT_MASS) || (pPhysObj->GetMass() < ANTLIONWARRIOR_MIN_OBJECT_MASS))
			continue;

		// Tell the AI sensing list that we want to consider this
		g_AI_SensedObjectsManager.AddEntity(pObject);

		if (g_debug_antlionwarrior.GetInt() == 5)
		{
...
//-----------------------------------------------------------------------------
bool CNPC_AntlionWarrior::CanSummon(bool bIgnoreTime)
{
	if (!m_bBarkEnabled)
		return false;
	...
	if (m_iNumLiveAntlions >= MAX(1, ANTLIONWARRIOR_SUMMON_COUNT - 1))
		return false;
...
//-----------------------------------------------------------------------------
int CNPC_AntlionWarrior::SelectUnreachableSchedule(void)
{
	// If we're in the cavern setting, we opt out of this
	if (m_bInCavern)
		return SCHED_ANTLIONWARRIOR_CHASE_ENEMY_TOLERANCE;
	// Summon antlions if we can
	if (HasCondition(COND_ANTLIONWARRIOR_CAN_SUMMON))
	...
	if (pTarget)
	{
		...
		SetCondition(COND_ANTLIONWARRIOR_PHYSICS_TARGET);
		...
		if (GetSquad() != NULL)
		{
			GetSquad()->BroadcastInteraction(g_interactionAntlionWarriorFoundPhysicsObject, (void *)((CBaseEntity *)m_hPhysicsTarget), this);
		}

		return SCHED_ANTLIONWARRIOR_PHYSICS_ATTACK;
	}
	...
	if (HasCondition(COND_SEE_ENEMY))
	{
		...
		if (GetEnemy() && HasCondition(COND_HEAVY_DAMAGE))
		{
			...
			return SCHED_ANTLIONWARRIOR_TAKE_COVER_FROM_ENEMY;
		}
	}
	...
	return SCHED_ANTLIONWARRIOR_CANT_ATTACK;
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
int CNPC_AntlionWarrior::SelectCombatSchedule(void)
{
	ClearHintGroup();
	...
	*/
	...
	if (HasCondition(COND_ANTLIONWARRIOR_CAN_SUMMON))
	{
		...
		if (HasCondition(COND_ANTLIONWARRIOR_CAN_CHARGE) && m_iNumLiveAntlions)
	...
	//Physics target
	if (HasCondition(COND_ANTLIONWARRIOR_PHYSICS_TARGET) && !m_bInCavern)
		return SCHED_ANTLIONWARRIOR_PHYSICS_ATTACK;
...
//-----------------------------------------------------------------------------
bool CNPC_AntlionWarrior::ShouldCharge(const Vector &startPos, const Vector &endPos, bool useTime, bool bCheckForCancel)
{
	...
	if (!bCheckForCancel)
	{
		...
		// Must be within our tolerance range
		if ((distance < ANTLIONWARRIOR_CHARGE_MIN) || (distance > ANTLIONWARRIOR_CHARGE_MAX))
			return false;
	}
	...
	// Draw the probe
	if (g_debug_antlionwarrior.GetInt() == 1)
	{
	...
	if (IsMoveBlocked(moveTrace))
	{
		...
		if (UTIL_DistApprox(WorldSpaceCenter(), moveTrace.vEndPosition) < ANTLIONWARRIOR_CHARGE_MIN)
			return false;
	...
	if ( useTime )
	{
	m_flChargeTime = gpGlobals->curtime + 0.2f;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
int CNPC_AntlionWarrior::SelectSchedule(void)
{
	...
	if (0) //g_debug_antlionwarrior.GetInt() == 3 )
	{
		m_flPhysicsCheckTime = 0;
	...
	if (!m_bInCavern && HasCondition(COND_HEAVY_DAMAGE) && m_flNextHeavyFlinchTime < gpGlobals->curtime)
	{
		...
		return SCHED_ANTLIONWARRIOR_PHYSICS_DAMAGE_HEAVY;
	}
	...
	if (m_bPreferPhysicsAttack)
	{
		...
		if (HasCondition(COND_ANTLIONWARRIOR_PHYSICS_TARGET))
			return SCHED_ANTLIONWARRIOR_PHYSICS_ATTACK;
	}
	...
	{
		ClearCondition(COND_ANTLIONWARRIOR_HAS_CHARGE_TARGET);
		ClearHintGroup();
...
// Purpose: 
//-----------------------------------------------------------------------------
int CNPC_AntlionWarrior::MeleeAttack1Conditions(float flDot, float flDist)
{
	...
	if (IsCurSchedule(SCHED_ANTLIONWARRIOR_CHARGE))
		return 0;
	...
	if (hl2_episodic.GetBool() && m_bInCavern)
	{
		...
		if ((GetAbsOrigin() - vecPredPos).Length() > ANTLIONWARRIOR_MELEE1_RANGE)
			return COND_TOO_FAR_TO_ATTACK;
	}
	else
	{
		// Must be close enough
		if (flDist > ANTLIONWARRIOR_MELEE1_RANGE)
			return COND_TOO_FAR_TO_ATTACK;
	}

	// Must be within a viable cone
	if (flDot < ANTLIONWARRIOR_MELEE1_CONE)
...
// Purpose: 
//-----------------------------------------------------------------------------
float CNPC_AntlionWarrior::MaxYawSpeed(void)
{
	Activity eActivity = GetActivity();

	// Stay still
	if ((eActivity == ACT_ANTLIONWARRIOR_SEARCH) ||
		(eActivity == ACT_ANTLIONWARRIOR_PEEK_ENTER) ||
		(eActivity == ACT_ANTLIONWARRIOR_PEEK_EXIT) ||
		(eActivity == ACT_ANTLIONWARRIOR_PEEK1) ||
		(eActivity == ACT_ANTLIONWARRIOR_BARK) ||
		(eActivity == ACT_ANTLIONWARRIOR_PEEK_SIGHTED) ||
	...
	if (eActivity == ACT_ANTLIONWARRIOR_CHARGE_START)
		return 10.0f;

	if (hl2_episodic.GetBool() && m_bInCavern)
	{
		// Allow a better turning rate when moving quickly but not charging the player
		if ((eActivity == ACT_ANTLIONWARRIOR_CHARGE_RUN) && IsCurSchedule(SCHED_ANTLIONWARRIOR_CHARGE) == false)
			return 0.0f;
	}

	// Turn more slowly as we close in on our target
	if (eActivity == ACT_ANTLIONWARRIOR_CHARGE_RUN)
	{
		if (pEnemy == NULL)
			return 0.0f;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWarrior::OverrideMove(float flInterval)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::Shove(void)
{
	if (GetNextAttack() > gpGlobals->curtime)
		return;
	...
	float damage = (pTarget->IsPlayer()) ? sk_antlionwarrior_dmg_shove.GetFloat() : 250;
	...
	if (flDistSqr < (ANTLIONWARRIOR_MELEE1_RANGE*ANTLIONWARRIOR_MELEE1_RANGE) && flDot >= ANTLIONWARRIOR_MELEE1_CONE)
	{
		vecEnd = pTarget->WorldSpaceCenter();
	}
	else
	{
		vecEnd = WorldSpaceCenter() + (BodyDirection3D() * ANTLIONWARRIOR_MELEE1_RANGE);
	}
	...
	if (pHurt)
	{
		...
		if (pHurt->IsPlayer())
		{
			...
			if (m_bInCavern && pHurt->IsPlayer() && pHurt->IsAlive() && pHurt->m_iHealth > ANTLIONWARRIOR_POISON_TO)
			{
				// That didn't finish them. Take them down to one point with poison damage. It'll heal.
				pHurt->TakeDamage(CTakeDamageInfo(this, this, pHurt->m_iHealth - ANTLIONWARRIOR_POISON_TO, DMG_POISON));
			}
#endif
...
Vector CheckThrowTolerance(CBaseEntity *pEdict, const Vector &vecSpot1, Vector vecSpot2, float flSpeed, float flTolerance)
{
...
public:
	...
	CTraceFilterCharge(const IHandleEntity *passentity, int collisionGroup, CNPC_AntlionWarrior *pAttacker)
	...
	virtual bool ShouldHitEntity(IHandleEntity *pHandleEntity, int contentsMask)
	{
		...
		if (pEntity)
		{
			...
			float	flDamage = (pEntity->IsPlayer()) ? sk_antlionwarrior_dmg_shove.GetFloat() : 250;;
...
public:
	...
	CNPC_AntlionWarrior	*m_pAttacker;
};
...
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::Footstep(bool bHeavy)
{
...
//			*pOut - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::GetPhysicsShoveDir(CBaseEntity *pObject, float flMass, Vector *pOut)
{
...
bool CNPC_AntlionWarrior::IsCharging()
{
	if (GetActivity() == ACT_ANTLIONWARRIOR_CHARGE_RUN)
		return true;
	if (GetActivity() == ACT_ANTLIONWARRIOR_CHARGE_START)
		return true;
	if (GetActivity() == ACT_ANTLIONWARRIOR_CHARGE_ANTICIPATION)
		return true;
...
// Input  : *pEvent - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::HandleAnimEvent(animevent_t *pEvent)
{
	if (pEvent->event == AE_ANTLIONWARRIOR_CHARGE_EARLYOUT)
	{
		...
		{
		SetSchedule( SCHED_ANTLIONWARRIOR_CHARGE_CANCEL );
		}
		*/
	...
	if (pEvent->event == AE_ANTLIONWARRIOR_SPIT)
	{
		if (GetEnemy())
		{
			...
			for (int i = 0; i < 3; i++)
			{
				CHLRFireball *pGrenade = (CHLRFireball*)CreateEntityByName("hlr_fireball");
				...
				{
					pGrenade->SetAbsVelocity((vecToss + RandomVector(-0.1f, 0.1f)) * flVelocity);
					//pGrenade->SetSpitSize();
				}
	...
	if (pEvent->event == AE_ANTLIONWARRIOR_SHOVE_PHYSOBJECT)
	{
		if (m_hPhysicsTarget == NULL)
		{
		...
		ClearCondition(COND_ANTLIONWARRIOR_PHYSICS_TARGET);
		ClearCondition(COND_ANTLIONWARRIOR_PHYSICS_TARGET_INVALID);
		...
		if (GetSquad() != NULL)
		{
			GetSquad()->BroadcastInteraction(g_interactionAntlionWarriorShovedPhysicsObject, (void *)((CBaseEntity *)m_hPhysicsTarget), this);
		}
	...
	if (pEvent->event == AE_ANTLIONWARRIOR_CHARGE_HIT)
	{
	...
	if (pEvent->event == AE_ANTLIONWARRIOR_SHOVE)
	{
	...
	if (pEvent->event == AE_ANTLIONWARRIOR_FOOTSTEP_LIGHT)
	{
		if (HasSpawnFlags(SF_ANTLIONWARRIOR_INSIDE_FOOTSTEPS))
		{
#if HL2_EPISODIC
	...
	if (pEvent->event == AE_ANTLIONWARRIOR_FOOTSTEP_HEAVY)
	{
		if (HasSpawnFlags(SF_ANTLIONWARRIOR_INSIDE_FOOTSTEPS))
		{
#if HL2_EPISODIC
	...
	if (pEvent->event == AE_ANTLIONWARRIOR_VOICE_GROWL)
	{
		StartSounds();
		...
		{
			duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionWarriorFastGrowl, ARRAYSIZE(envAntlionWarriorFastGrowl));
		}
		else
		{
			duration = 1.0f;
	...
	if (pEvent->event == AE_ANTLIONWARRIOR_VOICE_BARK)
	{
		StartSounds();

		float duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionWarriorBark1, ARRAYSIZE(envAntlionWarriorBark1));
		ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pConfusedSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionWarriorBark2, ARRAYSIZE(envAntlionWarriorBark2));
	...
	if (pEvent->event == AE_ANTLIONWARRIOR_VOICE_ROAR)
	{
		StartSounds();

		float duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionWarriorFastGrowl, ARRAYSIZE(envAntlionWarriorFastGrowl));
	...
	if (pEvent->event == AE_ANTLIONWARRIOR_VOICE_PAIN)
	{
		StartSounds();

		float duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pConfusedSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionWarriorPain1, ARRAYSIZE(envAntlionWarriorPain1));
		ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionWarriorBark2, ARRAYSIZE(envAntlionWarriorBark2));
	...
	if (pEvent->event == AE_ANTLIONWARRIOR_VOICE_SQUEEZE)
	{
		StartSounds();

		float duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionWarriorSqueeze, ARRAYSIZE(envAntlionWarriorSqueeze));
	...
	if (pEvent->event == AE_ANTLIONWARRIOR_VOICE_SCRATCH)
	{
		StartSounds();
	...
	if (pEvent->event == AE_ANTLIONWARRIOR_VOICE_GRUNT)
	{
		StartSounds();

		float duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pConfusedSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionWarriorGrunt, ARRAYSIZE(envAntlionWarriorGrunt));
		ENVELOPE_CONTROLLER.SoundPlayEnvelope(m_pGrowlHighSound, SOUNDCTRL_CHANGE_VOLUME, envAntlionWarriorGrunt2, ARRAYSIZE(envAntlionWarriorGrunt2));
	...
	if (pEvent->event == AE_ANTLIONWARRIOR_BURROW_OUT)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::SetHeavyDamageAnim(const Vector &vecSource)
{
	if (!m_bInCavern)
	{
		...
		if (damageDot > 0.0f)
		{
			...
			{
				m_nFlinchActivity = ACT_ANTLIONWARRIOR_PHYSHIT_FR;
			}
			else
			{
				m_nFlinchActivity = ACT_ANTLIONWARRIOR_PHYSHIT_FL;
			}
		}
		else
		{
			...
			if (damageRightDot < 0.0f)
			{
				m_nFlinchActivity = ACT_ANTLIONWARRIOR_PHYSHIT_RR;
			}
			else
			{
				m_nFlinchActivity = ACT_ANTLIONWARRIOR_PHYSHIT_RL;
			}
		}
	}
}
...
// Input  : &info - 
//-----------------------------------------------------------------------------
int CNPC_AntlionWarrior::OnTakeDamage_Alive(const CTakeDamageInfo &info)
{
	CTakeDamageInfo dInfo = info;

	// Don't take damage from another antlion guard!
	if (dInfo.GetAttacker() && dInfo.GetAttacker() != this && FClassnameIs(dInfo.GetAttacker(), "npc_antlionwarrior"))
		return 0;
...
//			bitsDamageType - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::TraceAttack(const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator)
{
...
// Input  : *pTask - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::StartTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_ANTLIONWARRIOR_SET_FLINCH_ACTIVITY:
	...
	case TASK_ANTLIONWARRIOR_SHOVE_PHYSOBJECT:
	{
		...
		SetActivity(ACT_ANTLIONWARRIOR_SHOVE_PHYSOBJECT);
	}
	break;

	case TASK_ANTLIONWARRIOR_FIND_PHYSOBJECT:
	{
	...
	case TASK_ANTLIONWARRIOR_GET_PATH_TO_PHYSOBJECT:
	{
		if ((m_hPhysicsTarget == NULL) || (GetEnemy() == NULL))
		{
	...
	case TASK_ANTLIONWARRIOR_CHARGE:
	{
		...
		GetMotor()->MoveStop();
		bCharging = true;
		SetActivity(ACT_ANTLIONWARRIOR_CHARGE_START);
		m_bDecidedNotToStop = false;
	}
	break;


	case TASK_ANTLIONWARRIOR_GET_PATH_TO_CHARGE_POSITION:
	{
	...
	case TASK_ANTLIONWARRIOR_GET_PATH_TO_NEAREST_NODE:
	{
		if (!GetEnemy())
		{
	...
	case TASK_ANTLIONWARRIOR_GET_CHASE_PATH_ENEMY_TOLERANCE:
	{
	...
	case TASK_ANTLIONWARRIOR_OPPORTUNITY_THROW:
	{
		// If we've got some live antlions, look for a physics object to throw
		if (m_iNumLiveAntlions >= 2 && RandomFloat(0, 1) > 0.5)
		{
			m_FailedPhysicsTargets.RemoveAll();
			UpdatePhysicsTarget(false, m_bPreferPhysicsAttack ? (100 * 12) : ANTLIONWARRIOR_FARTHEST_PHYSICS_OBJECT);
			if (HasCondition(COND_ANTLIONWARRIOR_PHYSICS_TARGET) && !m_bInCavern)
			{
				SetSchedule(SCHED_ANTLIONWARRIOR_PHYSICS_ATTACK);
			}
		}
...
//-----------------------------------------------------------------------------
void ApplySprintDamage(CBaseEntity *pAntlionWarrior, CBaseEntity *pTarget, float flDamage)
{
	Vector attackDir = (pTarget->WorldSpaceCenter() - pAntlionWarrior->WorldSpaceCenter());
	VectorNormalize(attackDir);
	...
	// Deal the damage
	CTakeDamageInfo	info(pAntlionWarrior, pAntlionWarrior, vecForce, offset, flDamage, DMG_CLUB);
	pTarget->TakeDamage(info);

#if HL2_EPISODIC
	// If I am a cavern guard attacking the player, and he still lives, then poison him too.
	Assert(dynamic_cast<CNPC_AntlionWarrior *>(pAntlionWarrior));

	if (static_cast<CNPC_AntlionWarrior *>(pAntlionWarrior)->IsInCavern() && pTarget->IsPlayer() && pTarget->IsAlive() && pTarget->m_iHealth > ANTLIONWARRIOR_POISON_TO)
	{
		// That didn't finish them. Take them down to one point with poison damage. It'll heal.
		pTarget->TakeDamage(CTakeDamageInfo(pAntlionWarrior, pAntlionWarrior, pTarget->m_iHealth - ANTLIONWARRIOR_POISON_TO, DMG_POISON));
	}
#endif
	...
	virtual bool ShouldHitEntity(IHandleEntity *pHandleEntity, int contentsMask)
	{
		...
		if (pEntity)
		{
			...
			if (pEntity->Classify() == CLASS_ANTLION)
			{
				...
				ApplySprintDamage(pGuard, pEntity, pEntity->GetHealth());
				return false;
			}
		}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWarrior::EnemyIsRightInFrontOfMe(CBaseEntity **pEntity)
{
	if (!GetEnemy())
		return false;
...
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::ChargeLookAhead(void)
{
	trace_t	tr;
	Vector vecForward;
	...
	if (tr.fraction != 1.0)
	{
		...
		AddGesture(ACT_ANTLIONWARRIOR_CHARGE_ANTICIPATION);
	}
}
...
//-----------------------------------------------------------------------------
bool CNPC_AntlionWarrior::HandleChargeImpact(Vector vecImpact, CBaseEntity *pEntity)
{
	...
	if ((IRelationType(pEntity) == D_NU || D_HT) && (GetNextAttack() < gpGlobals->curtime))
	{
		...
		if (!IsPlayingGesture(ACT_ANTLIONWARRIOR_CHARGE_HIT))
		{
			RestartGesture(ACT_ANTLIONWARRIOR_CHARGE_HIT);
		}
		...
		SetActivity(ACT_ANTLIONWARRIOR_CHARGE_STOP);
...
// Output : float
//-----------------------------------------------------------------------------
float CNPC_AntlionWarrior::ChargeSteer(void)
{
	trace_t	tr;
...
// Input  : *pTask - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::RunTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_ANTLIONWARRIOR_SET_FLINCH_ACTIVITY:
	...
	case TASK_ANTLIONWARRIOR_CHARGE:
	{
		...
		if (eActivity == ACT_ANTLIONWARRIOR_CHARGE_STOP || eActivity == ACT_ANTLIONWARRIOR_CHARGE_CRASH)
		{
			if (IsActivityFinished())
			{
				TaskComplete();
		...
		if ((eActivity == ACT_ANTLIONWARRIOR_CHARGE_START) && (IsActivityFinished()))
		{
			SetActivity(ACT_ANTLIONWARRIOR_CHARGE_RUN);
		}

		// See if we're still running
		if (eActivity == ACT_ANTLIONWARRIOR_CHARGE_RUN || eActivity == ACT_ANTLIONWARRIOR_CHARGE_START)
		{
		...
		if (AutoMovement(GetEnemy(), &moveTrace) == false)
		{
			...
			if (HandleChargeImpact(moveTrace.vEndPosition, moveTrace.pObstruction))
			{
				...
				if (eActivity != ACT_ANTLIONWARRIOR_CHARGE_STOP)
				{
					...
					{
						SetActivity(ACT_ANTLIONWARRIOR_CHARGE_CRASH);
					}
				}
			}
			else if (moveTrace.pObstruction)
			{
				...
				{
					if (FClassnameIs(moveTrace.pObstruction, "npc_antlionwarrior"))
					{
						// Crash unless we're trying to stop already
						if (eActivity != ACT_ANTLIONWARRIOR_CHARGE_STOP)
						{
					...
					{
						ApplySprintDamage(this, moveTrace.pObstruction, moveTrace.pObstruction->GetHealth());
					}
				}
			}
		}
...
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::SummonAntlions(void)
{
	...
	int iSpawnPoint = 0;
	for (int i = 0; (m_iNumLiveAntlions < ANTLIONWARRIOR_SUMMON_COUNT) && (iSpawnPoint < ARRAYSIZE(sAntlionSpawnPositions)); i++)
	{
		...
		{
			if (g_debug_antlionwarrior.GetInt() == 2)
			{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::FoundEnemy(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::LostEnemy(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::InputSetShoveTarget(inputdata_t &inputdata)
{
	if (IsAlive() == false)
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::InputSetChargeTarget(inputdata_t &inputdata)
{
	if (!IsAlive())
		return;
	...
	SetCondition(COND_ANTLIONWARRIOR_HAS_CHARGE_TARGET);
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::InputClearChargeTarget(inputdata_t &inputdata)
{
	m_hChargeTarget = NULL;
}
...
// Output : Activity
//-----------------------------------------------------------------------------
Activity CNPC_AntlionWarrior::NPC_TranslateActivity(Activity baseAct)
{
	//See which run to use
	if ((baseAct == ACT_RUN) && IsCurSchedule(SCHED_ANTLIONWARRIOR_CHARGE))
		return (Activity)ACT_ANTLIONWARRIOR_CHARGE_RUN;
	...
	if ((baseAct == ACT_RUN) && (m_iHealth <= (m_iMaxHealth / 4)))
		return (Activity)ACT_ANTLIONWARRIOR_RUN_HURT;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWarrior::ShouldWatchEnemy(void)
{
	Activity nActivity = GetActivity();

	if ((nActivity == ACT_ANTLIONWARRIOR_SEARCH) ||
		(nActivity == ACT_ANTLIONWARRIOR_PEEK_ENTER) ||
		(nActivity == ACT_ANTLIONWARRIOR_PEEK_EXIT) ||
		(nActivity == ACT_ANTLIONWARRIOR_PEEK1) ||
		(nActivity == ACT_ANTLIONWARRIOR_PEEK_SIGHTED) ||
		(nActivity == ACT_ANTLIONWARRIOR_SHOVE_PHYSOBJECT) ||
		(nActivity == ACT_ANTLIONWARRIOR_PHYSHIT_FR) ||
		(nActivity == ACT_ANTLIONWARRIOR_PHYSHIT_FL) ||
		(nActivity == ACT_ANTLIONWARRIOR_PHYSHIT_RR) ||
		(nActivity == ACT_ANTLIONWARRIOR_PHYSHIT_RL) ||
		(nActivity == ACT_ANTLIONWARRIOR_CHARGE_CRASH) ||
		(nActivity == ACT_ANTLIONWARRIOR_CHARGE_HIT) ||
		(nActivity == ACT_ANTLIONWARRIOR_CHARGE_ANTICIPATION))
	{
		return false;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::UpdateHead(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::MaintainPhysicsTarget(void)
{
	if (m_hPhysicsTarget == NULL || GetEnemy() == NULL)
		return;
	...
	{
		ClearCondition(COND_ANTLIONWARRIOR_PHYSICS_TARGET);
		SetCondition(COND_ANTLIONWARRIOR_PHYSICS_TARGET_INVALID);
		m_hPhysicsTarget = NULL;
		return;
	}
	else
	...
	if (g_debug_antlionwarrior.GetInt() == 3)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::UpdatePhysicsTarget(bool bPreferObjectsAlongTargetVector, float flRadius)
{
	if (GetEnemy() == NULL)
		return;
	...
	criteria.flTargetCone = ANTLIONWARRIOR_OBJECTFINDING_FOV;
...
//-----------------------------------------------------------------------------
bool CNPC_AntlionWarrior::ShouldProbeCollideAgainstEntity(CBaseEntity *pEntity)
{
	...
	if (pEntity->GetMoveType() == MOVETYPE_VPHYSICS)
	{
		...
		if (pPhysObj && pPhysObj->GetMass() <= ANTLIONWARRIOR_MAX_OBJECT_MASS)
		{
			return false;
		}
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::PrescheduleThink(void)
{
	...
	if (IsCurSchedule(SCHED_ANTLIONWARRIOR_CHASE_ENEMY) ||
		IsCurSchedule(SCHED_ANTLIONWARRIOR_PATROL_RUN) ||
		IsCurSchedule(SCHED_ANTLIONWARRIOR_CANT_ATTACK) ||
		IsCurSchedule(SCHED_ANTLIONWARRIOR_CHASE_ENEMY_TOLERANCE))
	{
		bool bCheckAlongLine = (IsCurSchedule(SCHED_ANTLIONWARRIOR_CHASE_ENEMY) || IsCurSchedule(SCHED_ANTLIONWARRIOR_CHASE_ENEMY_TOLERANCE));
		UpdatePhysicsTarget(bCheckAlongLine);
	}
	else if (!IsCurSchedule(SCHED_ANTLIONWARRIOR_PHYSICS_ATTACK))
	{
		ClearCondition(COND_ANTLIONWARRIOR_PHYSICS_TARGET);
		m_hPhysicsTarget = NULL;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::GatherConditions(void)
{
	BaseClass::GatherConditions();

	if (CanSummon(false))
	{
		SetCondition(COND_ANTLIONWARRIOR_CAN_SUMMON);
	}
	else
	{
		ClearCondition(COND_ANTLIONWARRIOR_CAN_SUMMON);
	}
	...
	if (GetEnemy())
	{
		...
		{
			ClearCondition(COND_ANTLIONWARRIOR_CAN_CHARGE);
		}
	}
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::StopLoopingSounds()
{
	//Stop all sounds
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::InputUnburrow(inputdata_t &inputdata)
{
	if (IsAlive() == false)
		return;
	...
	SetSchedule(SCHED_ANTLIONWARRIOR_UNBURROW);
...
// Output  :
//------------------------------------------------------------------------------
bool CNPC_AntlionWarrior::IsUnreachable(CBaseEntity *pEntity)
{
...
//-----------------------------------------------------------------------------
Vector CNPC_AntlionWarrior::GetPhysicsHitPosition(CBaseEntity *pObject, CBaseEntity *pTarget, Vector *vecTrajectory, float *flClearDistance)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
inline bool CNPC_AntlionWarrior::CanStandAtPoint(const Vector &vecPos, Vector *pOut)
{
...
//-----------------------------------------------------------------------------
bool CNPC_AntlionWarrior::CanStandAtShoveTarget(CBaseEntity *pShoveObject, CBaseEntity *pTarget, Vector *pOut)
{
...
//-----------------------------------------------------------------------------
CBaseEntity *CNPC_AntlionWarrior::GetNextShoveTarget(CBaseEntity *pLastEntity, AISightIter_t &iter)
{
...
//-----------------------------------------------------------------------------
CBaseEntity *CNPC_AntlionWarrior::FindPhysicsObjectTarget(const PhysicsObjectCriteria_t &criteria)
{
...
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::BuildScheduleTestBits(void)
{
	...
	if (IsCurSchedule(SCHED_ANTLIONWARRIOR_CHASE_ENEMY))
	{
		SetCustomInterruptCondition(COND_ANTLIONWARRIOR_PHYSICS_TARGET);
		SetCustomInterruptCondition(COND_ANTLIONWARRIOR_CAN_SUMMON);
	}

	// Interrupt if we've been given a charge target
	if (IsCurSchedule(SCHED_ANTLIONWARRIOR_CHARGE) == false)
	{
		SetCustomInterruptCondition(COND_ANTLIONWARRIOR_HAS_CHARGE_TARGET);
	}
...
//			magnitude - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::ImpactShock(const Vector &origin, float radius, float magnitude, CBaseEntity *pIgnored)
{
...
// Input  : *pTarget - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::ChargeDamage(CBaseEntity *pTarget)
{
	if (pTarget == NULL)
		return;
	...
	float flDamage = (pPlayer == NULL) ? 250 : sk_antlionwarrior_dmg_charge.GetFloat();

	// If it's being held by the player, break that bond
	Pickup_ForcePlayerToDropThisObject(pTarget);

	// Calculate the physics force
	ApplySprintDamage(this, pTarget, flDamage);
}
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::InputRagdoll(inputdata_t &inputdata)
{
	if (IsAlive() == false)
		return;
...
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::InputEnablePreferPhysicsAttack(inputdata_t &inputdata)
{
...
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::InputDisablePreferPhysicsAttack(inputdata_t &inputdata)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
int CNPC_AntlionWarrior::SelectFailSchedule(int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode)
{
	// Figure out what to do next
	if (failedSchedule == SCHED_ANTLIONWARRIOR_CHASE_ENEMY && HasCondition(COND_ENEMY_UNREACHABLE))
		return SelectUnreachableSchedule();
...
// Output : int
//-----------------------------------------------------------------------------
int CNPC_AntlionWarrior::TranslateSchedule(int scheduleType)
{
	switch (scheduleType)
	{
	case SCHED_CHASE_ENEMY:
		return SCHED_ANTLIONWARRIOR_CHASE_ENEMY;
		break;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::StartSounds(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::InputEnableBark(inputdata_t &inputdata)
{
	m_bBarkEnabled = true;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::InputDisableBark(inputdata_t &inputdata)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::DeathSound(const CTakeDamageInfo &info)
{
...
// Input  : &info - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::Event_Killed(const CTakeDamageInfo &info)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWarrior::CanBecomeRagdoll(void)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWarrior::BecomeRagdollOnClient(const Vector &force)
{
	if (!CanBecomeRagdoll())
		return false;
	...
	if (m_spawnflags & SF_ANTLIONWARRIOR_SERVERSIDE_RAGDOLL)
	{
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_AntlionWarrior::OverrideMoveFacing(const AILocalMoveGoal_t &move, float flInterval)
{
...
#ifdef HL2_EPISODIC
	else if (GetEnemy() && IsCurSchedule(SCHED_ANTLIONWARRIOR_CANT_ATTACK))
	{
	...
	else if (GetEnemy() && GetNavigator()->GetMovementActivity() == ACT_RUN)
	{
		...
		if ((UTIL_DistApprox(vecEnemyLKP, GetAbsOrigin()) < 512) || IsCurSchedule(SCHED_ANTLIONWARRIOR_PATROL_RUN))
		{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWarrior::IsHeavyDamage(const CTakeDamageInfo &info)
{
	// Struck by blast
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWarrior::IsLightDamage(const CTakeDamageInfo &info)
{
	return false;
}
...
// Input  : *pChild - 
//-----------------------------------------------------------------------------
void CNPC_AntlionWarrior::InputSummonedAntlionDied(inputdata_t &inputdata)
{
	m_iNumLiveAntlions--;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWarrior::QueryHearSound(CSound *pSound)
{
...
//---------------------------------------------------------
bool CNPC_AntlionWarrior::CNavigator::GetStoppingPath(CAI_WaypointList *pClippedWaypoints)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWarrior::RememberFailedPhysicsTarget(CBaseEntity *pTarget)
{
	// Already in the list?
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_AntlionWarrior::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter *sender)
{
	// Don't chase targets that other guards in our squad may be going after!
	if (interactionType == g_interactionAntlionWarriorFoundPhysicsObject)
	{
	...
	if (interactionType == g_interactionAntlionWarriorShovedPhysicsObject)
	{
...
//-----------------------------------------------------------------------------
void	CNPC_AntlionWarrior::PopulatePoseParameters(void)
{
...
//-----------------------------------------------------------------------------
unsigned char CNPC_AntlionWarrior::GetBleedingLevel(void) const
{
...
AI_BEGIN_CUSTOM_NPC(npc_antlionwarrior, CNPC_AntlionWarrior)

// Interactions	
DECLARE_INTERACTION(g_interactionAntlionWarriorFoundPhysicsObject)
DECLARE_INTERACTION(g_interactionAntlionWarriorShovedPhysicsObject)

// Squadslots	
DECLARE_SQUADSLOT(SQUAD_SLOT_ANTLIONWARRIOR_CHARGE)

//Tasks
DECLARE_TASK(TASK_ANTLIONWARRIOR_CHARGE)
DECLARE_TASK(TASK_ANTLIONWARRIOR_GET_PATH_TO_PHYSOBJECT)
DECLARE_TASK(TASK_ANTLIONWARRIOR_SHOVE_PHYSOBJECT)
DECLARE_TASK(TASK_ANTLIONWARRIOR_SUMMON)
DECLARE_TASK(TASK_ANTLIONWARRIOR_SET_FLINCH_ACTIVITY)
DECLARE_TASK(TASK_ANTLIONWARRIOR_GET_PATH_TO_CHARGE_POSITION)
DECLARE_TASK(TASK_ANTLIONWARRIOR_GET_PATH_TO_NEAREST_NODE)
...
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_PEEK_FLINCH)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_PEEK_ENTER)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_PEEK_EXIT)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_PEEK1)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_BARK)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_PEEK_SIGHTED)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_CHARGE_START)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_CHARGE_CANCEL)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_CHARGE_RUN)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_CHARGE_CRASH)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_CHARGE_STOP)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_CHARGE_HIT)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_CHARGE_ANTICIPATION)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_SHOVE_PHYSOBJECT)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_FLINCH_LIGHT)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_UNBURROW)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_ROAR)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_RUN_HURT)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_PHYSHIT_FR)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_PHYSHIT_FL)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_PHYSHIT_RR)
DECLARE_ACTIVITY(ACT_ANTLIONWARRIOR_PHYSHIT_RL)

//Adrian: events go here
DECLARE_ANIMEVENT(AE_ANTLIONWARRIOR_CHARGE_HIT)
DECLARE_ANIMEVENT(AE_ANTLIONWARRIOR_SHOVE_PHYSOBJECT)
DECLARE_ANIMEVENT(AE_ANTLIONWARRIOR_SHOVE)
DECLARE_ANIMEVENT(AE_ANTLIONWARRIOR_FOOTSTEP_LIGHT)
DECLARE_ANIMEVENT(AE_ANTLIONWARRIOR_FOOTSTEP_HEAVY)
DECLARE_ANIMEVENT(AE_ANTLIONWARRIOR_CHARGE_EARLYOUT)
DECLARE_ANIMEVENT(AE_ANTLIONWARRIOR_VOICE_GROWL)
DECLARE_ANIMEVENT(AE_ANTLIONWARRIOR_VOICE_BARK)
DECLARE_ANIMEVENT(AE_ANTLIONWARRIOR_VOICE_PAIN)
DECLARE_ANIMEVENT(AE_ANTLIONWARRIOR_VOICE_SQUEEZE)
DECLARE_ANIMEVENT(AE_ANTLIONWARRIOR_VOICE_SCRATCH)
DECLARE_ANIMEVENT(AE_ANTLIONWARRIOR_VOICE_GRUNT)
...
"		TASK_STOP_MOVING						0"
"		TASK_FACE_ENEMY							0"
"		TASK_PLAY_SEQUENCE						ACTIVITY:ACT_ANTLIONWARRIOR_BARK"
"		TASK_ANTLIONWARRIOR_SUMMON				0"
"		TASK_ANTLIONWARRIOR_OPPORTUNITY_THROW		0"
"	"
"	Interrupts"
"		COND_HEAVY_DAMAGE"
)
...
"	Tasks"
"		TASK_STOP_MOVING					0"
"		TASK_SET_FAIL_SCHEDULE				SCHEDULE:SCHED_ANTLIONWARRIOR_CHASE_ENEMY"
"		TASK_FACE_ENEMY						0"
"		TASK_ANTLIONWARRIOR_CHARGE			0"
""
"	Interrupts"
"		COND_TASK_FAILED"
//"		COND_HEAVY_DAMAGE"
...
//==================================================
// SCHED_ANTLIONWARRIOR_CHARGE_SMASH
//==================================================
...
"	Tasks"
"		TASK_STOP_MOVING			0"
"		TASK_PLAY_SEQUENCE			ACTIVITY:ACT_ANTLIONWARRIOR_CHARGE_CRASH"
""
"	Interrupts"
"		COND_TASK_FAILED"
//"		COND_HEAVY_DAMAGE"
...
"	Tasks"
"		TASK_SET_FAIL_SCHEDULE						SCHEDULE:SCHED_ANTLIONWARRIOR_CHASE_ENEMY"
"		TASK_ANTLIONWARRIOR_GET_PATH_TO_PHYSOBJECT	0"
"		TASK_RUN_PATH								0"
"		TASK_WAIT_FOR_MOVEMENT						0"
"		TASK_FACE_ENEMY								0"
"		TASK_ANTLIONWARRIOR_SHOVE_PHYSOBJECT			0"
""
"	Interrupts"
"		COND_TASK_FAILED"
"		COND_ENEMY_DEAD"
...
"	Tasks"
"		TASK_SET_FAIL_SCHEDULE						SCHEDULE:SCHED_ANTLIONWARRIOR_CANT_ATTACK"
"		TASK_ANTLIONWARRIOR_FIND_PHYSOBJECT			0"
"		TASK_SET_SCHEDULE							SCHEDULE:SCHED_ANTLIONWARRIOR_PHYSICS_ATTACK"
""
"	Interrupts"
"		COND_ANTLIONWARRIOR_PHYSICS_TARGET"
//"		COND_HEAVY_DAMAGE"
)
...
"		TASK_ANTLIONWARRIOR_SET_FLINCH_ACTIVITY	0"
""
"	Interrupts"
)
...
"	Tasks"
"		TASK_PLAY_SEQUENCE			ACTIVITY:ACT_ANTLIONWARRIOR_UNBURROW"
""
"	Interrupts"
)
...
"	Tasks"
"		TASK_PLAY_SEQUENCE			ACTIVITY:ACT_ANTLIONWARRIOR_CHARGE_CANCEL"
""
"	Interrupts"
)
...
"	Tasks"
"		TASK_ANTLIONWARRIOR_GET_PATH_TO_CHARGE_POSITION	0"
"		TASK_RUN_PATH									0"
"		TASK_WAIT_FOR_MOVEMENT							0"
"	"
"	Interrupts"
...
"	Tasks"
"		TASK_STOP_MOVING									0"
"		TASK_SET_FAIL_SCHEDULE								SCHEDULE:SCHED_ANTLIONWARRIOR_PATROL_RUN"
"		TASK_ANTLIONWARRIOR_GET_PATH_TO_NEAREST_NODE			500"
"		TASK_RUN_PATH										0"
"		TASK_WAIT_FOR_MOVEMENT								0"
"		TASK_FACE_ENEMY										0"
""
...
"		TASK_STOP_MOVING			0"
"		TASK_FACE_ENEMY				0"
"		TASK_PLAY_SEQUENCE			ACTIVITY:ACT_ANTLIONWARRIOR_ROAR"
"	"
"	Interrupts"
"		COND_HEAVY_DAMAGE"
)
