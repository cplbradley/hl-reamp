...
static ConVar r_3dsky("r_3dsky", "1", 0, "Enable the rende	ring of 3d sky boxes");
...
static ConVar cl_showviewprojmatrix("cl_showviewprojmatrix", "0");
extern ClientRender_Globals_t g_pRenderGlobals;
...
//----------------------------------------------------------------------------
// Proper depth buffer from Vance
//----------------------------------------------------------------------------
...
void CViewRender::SetupMain3DView(const CViewSetup& view, int& nClearFlags)
{
	...
	MatrixMultiply(viewmatrix, projmatrix, m_ViewProjectionMatrix);
	MatrixInverseGeneral(m_ViewProjectionMatrix, m_InverseViewProjectionMatrix);
	pRenderContext.SafeRelease();

	if (cl_showviewprojmatrix.GetBool())
	{
		for (int i = 0; i < 4; i++)
		{
			engine->Con_NPrintf(i + 1, "%f %f %f %f \n", g_pRenderGlobals.curViewProjection[i][1], g_pRenderGlobals.curViewProjection[i][2], g_pRenderGlobals.curViewProjection[i][3], g_pRenderGlobals.curViewProjection[i][4]);
		}
	}
...
void CViewRender::CleanupMain3DView(const CViewSetup& view)
{
	...
	pRenderContext->GetMatrix(MATERIAL_VIEW, &viewmatrix);
	pRenderContext->GetMatrix(MATERIAL_PROJECTION, &projmatrix);
	MatrixMultiply(viewmatrix, projmatrix, m_LastViewProjectionMatrix);
	...
	if (cl_showviewprojmatrix.GetBool())
	{
		for (int i = 0; i < 4; i++)
		{
			engine->Con_NPrintf(i + 6, "%f %f %f %f \n", g_pRenderGlobals.lastViewProjection[i][1], g_pRenderGlobals.lastViewProjection[i][2], g_pRenderGlobals.lastViewProjection[i][3], g_pRenderGlobals.lastViewProjection[i][4]);
		}
	}
	render->PopView(GetFrustum());
}
...
void CViewRender::PushGlobals(ClientRender_Globals_t globals)
{
	MatrixCopy(m_ViewProjectionMatrix, g_pRenderGlobals.curViewProjection);
	MatrixCopy(m_InverseViewProjectionMatrix, g_pRenderGlobals.inverseViewProjection);
	MatrixCopy(m_LastViewProjectionMatrix, g_pRenderGlobals.lastViewProjection);

}

void CViewRender::SetupGlobals(const CViewSetup& view)
{
	ClientRender_Globals_t globals
	{
		m_LastViewProjectionMatrix, m_ViewProjectionMatrix, m_InverseViewProjectionMatrix
	};

	CMatRenderContextPtr pRenderContext(materials);
	ICallQueue* pCallQueue = pRenderContext->GetCallQueue();
	if (pCallQueue)
		pCallQueue->QueueCall(this, &CViewRender::PushGlobals, globals);
	else
		this->PushGlobals(globals);
	pRenderContext->SetIntRenderingParameter(INT_RENDERPARM_GLOBALS_PTR, int(&g_pRenderGlobals));
